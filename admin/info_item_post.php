...
// print_r($_FILES);
$info_item_image = $_FILES['info_item_image']['name'];
...
if (empty($sub_head)) {
    ...
    header('location: info_item.php');
} elseif (empty($info_item_image)) {
    $_SESSION['error_msg_image'] = "Image is Requried";
    header('location: info_item.php');
} else {

    $upload_image_orginal_name = $_FILES['info_item_image']['name'];
    $upload_image_orginal_size = $_FILES['info_item_image']['size'];
    if ($upload_image_orginal_size <= 2000000) {
        ...
        if (in_array($image_extention, $allow_image_extention)) {
            $insert_query = "INSERT INTO `company_infos`(`white_head`, `green_head`, `sub_head`, `para_one`, `para_two`, `image_location`) VALUES ('$white_head','$green_head','$sub_head','$para_one','$para_two','Primary Location')";
            ...
            $update_image_location_query = "UPDATE company_infos SET image_location='$image_location' WHERE id=$id_from_db";
            mysqli_query($db_conect, $update_image_location_query);
            $_SESSION['update_success'] = "Info Item Update Sucessfully!!";

            header('location: info_item.php');
        } else {
            $_SESSION['error_msg_image'] = "Plase Valid Extention Png,Jpg,Jpeg";
            header('location: info_item.php');
        }
    } else {
        $_SESSION['error_msg_image'] = "Image File Is too Big";
        header('location: info_item.php');
    }
}
